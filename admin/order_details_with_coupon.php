...
if(get_rst("select * from ord_summary where cart_id=0$cart_id",$row_s,$rst_s)){
	//$cart_id = $row_s["cart_id"];
	$delivery_status = $row_s["delivery_status"];
	$pay_status = $row_s["pay_status"];
}else{
	header('location: order_list_with_coupon_.php');
	js_redirect("order_list_with_coupon.php");
}
...
function print_page(divID){
	...
	do{
		...
		$sup_name="";
		$sup_alias="";
		if(get_rst("select sup_company,sup_alias, sup_vat from sup_mast where sup_id=0$sup_id",$row_sup)){
			$sup_name=$row_sup["sup_company"]; 
			$sup_alias=$row_sup["sup_alias"];
			$sup_gst_num = $row_sup["sup_vat"];
		}
		?>
	...
	<table width="80%"  border="0" class="list"  align="center">
		...
		if(get_rst("select coupon_id, coupon_amt,coupon_deduct from ord_coupon where cart_id=$cart_id", $row_c)){
			...
			<tr class="border_bottom">
				<td align="right" class="table-bg"><strong><span class="green-basket">Total Tax:</span></strong></td>
	...
	<?php
		...
		<tr>		
			<td align="right" class="table-bg"><strong><span class="green-basket">Total Amount Payable(Round-Off):</span></strong></td>
			<td align="right" width="100px" class="table-bg"><strong><div style="display: inline-block;" class="rupee-sign">&#8377; </div> <?=round($ord_pay_total,0,PHP_ROUND_HALF_UP)?></strong>&nbsp;</td>
		</tr>
		<?php }else{?>
		<tr>
			<td align="right" class="table-bg"><strong><span class="green-basket">Total Amount Payable:</span></strong></td>
			<td align="right" width="100px" class="table-bg"><strong><div style="display: inline-block;" class="rupee-sign">&#8377; </div> <?=FormatNumber($ord_pay_total - ($coupon_amt + $coupon_tax),2)?></strong>&nbsp;</td>
		</tr>
		<tr>
			<td align="right" class="table-bg"><strong><span class="green-basket">Total Amount Payable (Round-Off):</span></strong></td>
			<td align="right" width="100px" class="table-bg"><strong><div style="display: inline-block;" class="rupee-sign">&#8377; </div> <?=round($ord_pay_total - ($coupon_amt + $coupon_tax),0,PHP_ROUND_HALF_UP)?></strong>&nbsp;</td>
		<tr>
		<?php }?>
	</tr>
	</table>
	...
	<div id="ord_info">
		...
		<table width="100%"  border="1" align="center" class="checkout" >
			...
			<table width="100%"  border="1" class="list"  align="center">
				<tr><th>Supplier :&nbsp; <?=$sup_name?></th>
				<th><span>GST Number :&nbsp; <?=$sup_gst_num?><span></th></tr>
			</table>	
