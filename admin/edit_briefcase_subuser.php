...
$id = func_read_qs("id");
get_rst("select po_company_id from po_memb_mast where id=$id", $cmid);
$cid=$cmid["po_company_id"];
get_rst("select owner_id from po_company_mast where po_company_id=$cid",$uid);
$userid=$uid["owner_id"];
if($id<>""){
...
if(isset($_POST["submit"])){
	...
	if($msg=="1"){
		...
		if($id==""){
			...
			if($memb_count <=3){																		//check for max 3 users
				...
				if(!mysqli_query($con, $qry)){
					...
					get_rst("select memb_email,memb_tel from member_mast where memb_id=$userid",$r);
					get_rst("select company_id from po_company_mast where po_company_id=$cid",$rowcid);
				...
				$msg="You cannot create more than 3 user including yourself. For adding more user contact Company-Name Team.";
			}
		}else{																//update query
		...
		<script>
			alert("<?=$msg?>");
			window.location.href="manage_briefcase_sub-users.php";
		</script>
		<?php
