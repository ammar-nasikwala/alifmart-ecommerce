...
var global_msg = "";
var obj_prev = "";
function failed(obj,msg){
	//alert("H")
	try{
		//obj.style.backgroundColor = "#FF9189"
		obj.focus()

		if(obj_prev != obj){
			global_msg = global_msg + msg + "\n\r"
		}
		obj.style.border="solid 1px #FF9189"
...
//Loop through each element.
	 for (i=0; i<(f.length); i++)
	 {
		...
		//msg = " Kindly specify "+ t.title +" to allow us to complete our records."
		 //msg="The form is incomplete. Kindly enter in the " + t.title
		 msg="Kindly specify the " + t.title + "."

		 	 var vFlag = true
			//Store Name of the form elements
			var tval = t.id ;
			if(t.title=="Goods and Service Tax number (GSTN)" || t.title=="Establishment Name"){
				continue;
			}
			...
			if (myVal=="" && tval!="0000000" && fPos=="1" && t.type!="select-one"){
		 ...
		 switch (t.type)
	 	     ...
	 	     case "select-one":
	 	     {
				switch (fPos)
				{
					case "1":
					{
						...
						msg="Kindly select from the " + t.title + "." ;
			  			failed(t,msg)
						//alert (msg);
						//t.focus();
			  			//return false;
			 ...
			 case "checkbox":
			 {
				if(fPos=="1"){
					if(t.checked==false){
						failed(t,"Please tick the box for " + t.title + ".")
					}
				}
				break;
			 }
		     case "text": // If Input type is Text
		     {
			 		switch (fPos)
			 		{
						...
						if(parseFloat(lPos)>0)
						{
							...
							{
							//alert(msg);
							failed(t,"Kindly specify a valid number for " + t.title)
							//alert("Kindly specify a valid number for " + t.title);
							//t.focus();
							//return false;
							}
							...
							{
							failed(t,"Kindly specify a valid " + t.title)
						...
						if(sPos=="1"  && tPos=="1" && (!isEmpty(t.value)))
						{
							...
							{
								failed(t,"Kindly specify valid Date(dd/mm/yyyy) for " + t.title + ".")
								//alert("Kindly specify valid Date(dd/mm/yyyy) for " + t.title);
								//t.focus();
								//return false;
							}
							...
							{
							failed(t,"Kindly specify valid Date(mm/dd/yyyy) for " + t.title + ".")
							//alert("Kindly specify valid Date(mm/dd/yyyy) for " + t.title);
							//t.focus();
							//return false;
							}
							...
							if(sPos=="8" && (!(isPhone(t.value))))
							{
								...
								//alert("Invalid Phone no. for " + t.title);
								//t.focus();
								//return false;
							}
							...
							else if(sPos=="7" && (!(isWhitespace(t.value))))
							{
								...
								{
									failed(t,"Please provide a valid IP address eg:(192.168.1.202) for " + t.title)
									//alert("Please provide a valid address eg:(192.168.1.202) for " + t.title);
									//t.focus();
									//return false;
								}
								for(var n=0;n<countno.length;n++)
								{
									...
									{
										failed(t,"You can only enter a number.")
									...
									{
										failed(t,"Number must be less then 255 and greater then 0"+countno[n])
		 		...
		 		switch(fPos)
		 		{
		 			...
		 			case "1":
					{
						if(sPos=="0" && tPos=="1" && isWhitespace(t.value))
						  ...
						  if(t.value.length > parseFloat(lPos))
						  {
						  var newmsg = "You have exceeded the max limit for this field. \n\nKindly specify the " + t.title + " & it should not be more than " + parseFloat(lPos) + " characters ";
//						  var newmsg = "Maximum '" + parseFloat(lPos) + "' characters can be entered"
						  failed(t,newmsg)
						  //alert(newmsg);
						  //t.focus();
